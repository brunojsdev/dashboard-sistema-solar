...
# --- ESTILO CSS (Sua Paleta + Refinamentos Finais) ---
st.markdown("""
<style>
    /* Reset e Fundo */
    .stApp {
...
if st.session_state.selecao == "Geral":
    ...
    # Área de Filtros exclusiva para Visão Geral
    col_f1, _ = st.columns([2, 2])
    with col_f1:
        filtro_tipo = st.multiselect("Filtrar Exibição por Tipo:", options=df_original['Tipo'].unique(), default=df_original['Tipo'].unique())
    ...
    with col_right:
        ...
        # Troca de cores: Rochoso (Laranja) e Gelo (Roxo)
        # Gasoso mantido no centro como Destaque Claro
    ...
    # Descrição posicionada conforme solicitado
    ...
    with g1:
        ...
        # Compara selecionado com Terra e Júpiter (extremos)
        comp_df = df_original[df_original['Planeta'].isin(['Terra', 'Júpiter', target])].drop_duplicates()
